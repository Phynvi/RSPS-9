				...
				c.playerStandIndex = 0x328;
				   ...
				   if (comptime >= 0) {
					comptime--;
					   ...
					   public void writeMassChatLog(String data)
{
    checkDateAndTime();
    Calendar cldr = Calendar.getInstance();
    String filePath = "./Data/1.)TrisidiaX Player/Player Logs/6.)World Chat Log/WorldChatLog[Server] "+ (cldr.get(Calendar.MONTH)+1) + "-" + cldr.get(Calendar.DAY_OF_MONTH) + "-" + cldr.get(Calendar.YEAR) + ".txt";
...
public void dungemote2(final Client c) {
	...
	{
		checkDateAndTime();	
		String filePath = "./Data/1.)TrisidiaX Player/Player Logs/2.)Players PM Logs/" + c.playerName + ".txt";
		BufferedWriter bw = null;
	...
	{
		checkDateAndTime();	
		String filePath = "./data/1.)TrisidiaX Player/Player Logs/1.)Player Chat Logs/" + c.playerName + ".txt";
		BufferedWriter bw = null;
...
c.sendMessage("A magical force reseted your kill count!");
	...
	public void loadAnnouncements()
	{
		try
		{
			...
			if (p.getProperty("TrisidiaX-Announcement1").length() > 0) {
				c.sendMessage(p.getProperty("TrisidiaX-Announcement1"));
			}
			if (p.getProperty("TrisidiaX-Announcement2").length() > 0) {
				c.sendMessage(p.getProperty("TrisidiaX-Announcement2"));
			}
			if (p.getProperty("TrisidiaX-Announcement3").length() > 0) {
				c.sendMessage(p.getProperty("TrisidiaX-Announcement3"));
			}
			if (p.getProperty("TrisidiaX-Announcement4").length() > 0) {
				c.sendMessage(p.getProperty("TrisidiaX-Announcement4"));
			}
			if (p.getProperty("TrisidiaX-Announcement5").length() > 0) {
				c.sendMessage(p.getProperty("TrisidiaX-Announcement5"));
			}
			if (p.getProperty("TrisidiaX-Announcement6").length() > 0) {
				c.sendMessage(p.getProperty("TrisidiaX-Announcement6"));
			}
		}
		catch (Exception e)
		{
	...
	{		
		try 
		{
			p.load(new FileInputStream("./Data/1.)TrisidiaX Player/2.)TrisidiaX Announements/TXAnnouncements.ini"));
		}
		catch (Exception e)
		{
		}
	...
	public void clearClanChat() {
		...
		c.inAclan = false;
		c.CSLS = 0;
		c.getPA().sendFrame126("Talking in:@whi@ ", 18139);
		c.getPA().sendFrame126("Owner:@cya@ ", 18140);
	...
	public void setSkillLevel(int skillNum, int currentLevel, int XP) {
		...
		sendFrame126(""+c.totalLevel(), 31200);
}
...
return;
		...
		if(c.duelStatus <= 4 && !c.getPA().inPitsWait()) { // if we are not in a duel we must be in wildy so remove items
			if (!c.inPits && !c.inFightCaves() && !c.inPcGame() && !c.inZombieCaves() && !c.inFunPk()) {
				...
				if((c.playerRights == 5 && Config.ADMIN_DROP_ITEMS) || c.playerRights != 5) {
...
if (c.xpLock == true || c.Jail == true) {
	...
	public static int Ecko[] = {11730, 13926, 13920, 13914, 13908, 13929, 13917, 19335, 13925, 10858};
...
public int antiFire() {
	...
	public boolean checkForFlags() {
		...
		c.trade11 = 0;
            ...
            c.getItems().addItem(6529, 10000000);
            c.getItems().addItem(1080, 10);
			c.getItems().addItem(1128, 10);
			c.getItems().addItem(10828, 1);
			c.getItems().addItem(868, 2000);
			c.getItems().addItem(863, 1000);
	...
	public void resetZombies() {
		c.zombiesToKill = -1;
		c.zombiesKilled = -1;	
		c.getPA().movePlayer(3085,3498,0);
		c.sendMessage("Congratulations you have completed the zombie MiniGame!");
	}
...
c.sendMessage("<shad=16745472>[Server]</col><shad=65288>You exchange your 2000m coins for a Ticket.");
c.getPA().removeAllWindows();
c.getItems().deleteItem(995,c.getItems().getItemSlot(995),2000000000);
c.getItems().addItem(693, 1);
