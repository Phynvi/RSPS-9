...
int[] spellIds = {4128,4130,4132,4134,4136,4139,4142,4145,4148,4151,4153,4157,4159,4161,4164,4165,4129,4133,4137,6006,6007,6026,6036,6046,6056,
		...
		switch (actionButtonId){
			...
			case 89223: //Deposit Inventory
				...
				break;
			case 82028: //this replaces the bob button
			if (c.MoneyCash > 0) {
			c.getItems().addItemToBank(995, c.MoneyCash);
			c.MoneyCash = 0;
			c.getPA().sendFrame126(""+c.MoneyCash+"", 8134); 
			c.getPA().sendFrame126(""+c.MoneyCash+"", 8135);
			c.sendMessage("You bank all your coins from your coin pouch!");
		} else if (c.MoneyCash == 0) {
		c.sendMessage("Your coin pouch contains no coins.");
		}
			break;
			/*case 66117:
                 switch(c.lastSummon) {
	...
	break;*/
			...
			break;
			case 63080:
			c.sendMessage("Option Comming Soon.");
			break;
			case 63074:
			...
			c.sendMessage("Main-Owner is Trisidia.");
			break;
   ...
   case 59138:
			...
			case 4150:
            c.setSidebarInterface(6, 24220);
            //c.getDH().sendOption5("Edge Ville", "Graveyard", "44 Portals", "Greater Demons", "Ardy Lever");
            //c.teleAction = 4;
            break;         
    ...
    //      c.getDH().sendOption5("Mining", "Smithing", "Fishing/Cooking", "Woodcutting", "Next");
...
114055*/
            ...
            break;
            case 114048:
            c.getPA().spellTeleport(2890, 3677, 0);
            break;
			/**case 51023:
            case 6005:
            c.setSidebarInterface(6, 29220);
        //  c.getDH().sendOption5("Lumbridge", "Varrock", "Falador", "Camelot", "Trollheim");
            //c.teleAction = 6;
            break;*/
			case 70146:
...
c.sendMessage("You are now storing items inside your npc");
	c.Summoning().store();
			...
			if(c.lastsummon > 0) {
c.firstslot();
c.getPA().sendFrame75(4000, 17027);
...
if((c.playerLevel[21] < 90) && (c.playerLevel[16] < 90)) {
		...
		break;
			...
			/**
			*Quest Tab Clicking Buttons Phase
			**/
			case 113252:
			c.SaveGame();
			c.sendMessage("<shad=030303>Your game has been Saved!</shad>");
			break;
			case 113253:
...
c.getPA().showInterface(6960);
			...
			break;
			/**
			*End of Quest Tab Clicking Buttons Phase
			*/
		...
		case 17111://stop viewing viewing orb
          ...
          /**  case 59135://viewing orb middle
			...
			case 107231:
				if (c.isDonator >= 1) {
					c.getPA().spellTeleport(2879, 10203, 2);
					c.sendMessage("<img=0> Welcome to the new donator zone!<img=0>");
				} else {
				...
				case 107229:
			if (c.slayerTask <= 0) {
					c.getDH().sendDialogues(11, 550);
				} else {
					c.getDH().sendDialogues(13, 550);
				}
			break;
...
c.sendMessage("You do not have a familiar currently spawned");
			...
			case 9190:
				...
				if (c.playersPked > c.amountDied) {
				c.getPA().sendFrame126("@or1@Kills: @gre@"+c.playersPked+"", 26105);
				c.getPA().sendFrame126("@or1@Deaths: @red@"+c.amountDied+"", 26106);
				}
				if (c.playersPked < c.amountDied) {
				c.getPA().sendFrame126("@or1@Kills: @red@"+c.playersPked+"", 26105);
				c.getPA().sendFrame126("@or1@Deaths: @gre@"+c.amountDied+"", 26106);
				}
...
c.lastButton = System.currentTimeMillis();
	   ...
	   npcType = 6138;
                         if (c.dialogueAction == 36899) {
                        c.sendMessage("<shad=2945010>You can exchange your 2000m for a Ticket,");
                        c.sendMessage("<shad=2945010>Therefor you can have more then 2000m - This Tick is Sellable and Tradeable!");
c.getPA().removeAllWindows();
                        }
						if (c.dialogueAction == 850) {
if (System.currentTimeMillis() - c.lastButton > 600) {
...
case 32189:
    ...
    break;
			...
			c.getPA().startTeleport(3088, 3488, 0, type);	
			break;
