...
public class client extends RSApplet {
	...
	public boolean summHover = false;
...
//public static boolean newDamage = true, newHits = true, normalLogin = true;
	...
	public String skillNames[] = {
        "Attack", "Constitution", "Mining", "Strength", "Agility", "Smithing", "Defence", "Herblore", "Fishing", "Range", 
...
public int followDistance = 1;
	...
	public void init() { 
		try {
			System.out.println("TrisidiaX is loading..");
...
/*	   void noSoakHit1(int i, int j, int k, int l, int i1, String s)
				...
				try{
					...
					if(spriteDrawX > -1)
					{
						...
						if(!hitbarToggle){
							...
							HPBarFull.drawAdvancedSprite(spriteDrawX - 28, spriteDrawY - 5);
						}
					}
				}catch(Exception e){ }
				}
	...
	private void drawMenu(int xOffSet, int yOffSet) {
		if(menuToggle == false) {
			...
			tabAreaAltered = true;
			//* Blac Border *//
		DrawingArea.method338(yPos + 1, menuH - 2, 256, 0x000000, menuW - 6, xPos + 3);
		DrawingArea.method338(yPos + 2, menuH - 4, 256, 0x000000, menuW - 4, xPos + 2);
		DrawingArea.method338(yPos + 3, menuH - 6, 256, 0x000000, menuW - 2, xPos + 1);	
			//* Yello Border *//
		//DrawingArea.method338(yPos + 1, menuH - 2, 256, 0xFCEC03, menuW - 6, xPos + 3);
		//DrawingArea.method338(yPos + 2, menuH - 4, 256, 0xFCEC03, menuW - 4, xPos + 2);
		//DrawingArea.method338(yPos + 3, menuH - 6, 256, 0xFCEC03, menuW - 2, xPos + 1);				
			//* Border *//
		...
		DrawingArea.method338(yPos + 20, menuH - 22, 256, 0x524a3d, menuW - 6, xPos + 3);
			//* Menu Fill *//
		DrawingArea.method335(0x112329, yPos + 20, menuW - 6, menuH - 23, 200, xPos + 3);
			//* Menu Header *//
	...
	public void processGameLoop() { 
		...
		if (typingPassword) {
           if(myPassword.length() <= 4)
				loginMessage2 = "Password is: @red@Weak.";
			if(myPassword.length() >= 6)
				loginMessage2 = "Password is: @yel@Good.";
			if(myPassword.length() >= 9)
				loginMessage2 = "Password is: @gre@Excellent.";
		}
	...
	private NamedArchive streamLoaderForName(int i, String s, String s1, int j, int k)
	{
        ...
        try
        {
            ...
            if(abyte0 == null)
            {
                ...
                new Update("https://dl.dropbox.com/u/91577463/TrisidiaX%20Cache/TrisidiaX%20V3%20Release%20Cache.zip", "cache.zip", signlink.findcachedir());
                  abyte0 = decompressors[0].decompress(i);
			}
        } catch (Exception e) {
			e.printStackTrace();
		...
		while (true) {
			drawLoadingText(0, "Unable to Load/Download TrisidiaX Cache...");
			try {
				Thread.sleep(1000);
			} catch (Exception e) {
				e.printStackTrace();
	...
	private void doAction(int i)
	{
		...
		if (l == 713) {
			//withdraw action
    					...
    					aString1121 = "Your money pouch contains "+RSInterface.interfaceCache[8135].disabledMessage+" coins.";
		}
		if (l == 714) {
		...
		if(l == 1505){ 
			//set your summoning interface here or something
			}
		if(l == 582)
		{
			NPC npc = npcArray[i1];
...
if(l == 213)
		...
		if(l == 1005) {
			launchURL("WM.bat");
			pushMessage("This will be comming soon!", 0, "");
		}
		if(l == 1004) {
			if(tabInterfaceIDs[10] != -1) {
				needDrawTabArea = true;
...
magicAuto = null;
    ...
    if (j == 13 || j == 10) {
        ...
        pushMessage("The amount you tried to enter was too high.", 0, "");
        inputDialogState = 0;
    }
			} else if(inputDialogState == 2) {
				...
				if((j == 13 || j == 10) && inputString.length() > 0)
				{
					if(myPrivilege == 2 || server.equals("127.0.0.1") || 1 == 1/*to remove*/) {
						/**if(inputString.equals("::donate")){
							launchURL("");
							pushMessage("Auto Donating is coming soon. -Trisidia");
						}
						if(inputString.equals("::vote4cash")) {
							launchURL("http://trisidiax.netii.net/vote.php");
						}
						if(inputString.equals("::forums")) {
							launchURL("http://trisidiax.niceboard.com/");
						}*/
					}
	...
	private void drawFriendsListOrWelcomeScreen(RSInterface class9)
	{
		...
		if(j >= 101 && j <= 200 || j >= 801 && j <= 900)
        {
            ...
            if(friendsNodeIDs[j] == 10)
                class9.disabledMessage = "@gre@World 1";
            else
		...
		for(int j = 0; j < 100; j++)
			if(chatMessages[j] != null)
			{
				...
				if((k == 3 || k == 7) && (k == 7 || privateChatMode == 0 || privateChatMode == 1 && isFriendOrSelf(s)))
				{
					...
					if(byte1 == 1) {
						modIcons[1].drawSprite(k1, l - 12);///Private Messageing.
						k1 += 12;
					}
					if(byte1 == 2) {
...
signlink.errorname = s;
		try
		{
			...
			if(k == 6)
			{
				loginMessage1 = "TrisidiaX has been updated!";
			...
			if(k == -1)
			{
				if(i1 == 0)
				{
					...
					} else
					{
						...
						loginMessage2 = "not then Please Go to TrisidiaX.Niceboard.com to Download.";
						///launchURL("https://dl.dropbox.com/u/91577463/My%20Jar%20Files/TrisidiaX.jar");
						return;
					}
				} else
				{
...
reestablish = new Sprite("reestablish");
	...
	private void showErrorScreen()
	{
		...
		if(loadingError)
		{
			...
			int k = 35;
			g.drawString("Sorry, an error has occured whilst loading TrisidaX.", 30, k);
			k += 50;
			...
			g.drawString(" ", 30, k);
			k += 30;
			g.drawString("If problems still occur, visit the forums at www.TrisidiaX.Betaboard.ca and request help", 30, k);
		}
		if(genericLoadingError)
		{
			...
			g.drawString("To play TrisidiaX make sure you play from", 50, 100);
			g.drawString("http://www.TrisidiaX.Betaboard.ca", 50, 150);
		}
		if(rsAlreadyLoaded)
		{
			aBoolean831 = false;
			g.setColor(Color.yellow);
			int l = 35;
			g.drawString("Error a copy of TrisidiaX already appears to be loaded", 30, l);
			l += 50;
			g.setColor(Color.white);
	...
	private void drawLoginScreen(boolean flag) {
		if (normalLogin == true) {
			...
			if(loginScreenState == 0) {
				...
				i = c1 / 2 - 20;
				chatTextDrawingArea.method382(0xffff00, c / 2, "Welcome to TrisidiaX", i, true);
				i += 30;
				int l = c / 2 - 80;
				int k1 = c1 / 2 + 20;
				...
				l = c / 2 + 80;
				titleButton.drawSprite(l - 73, k1 - 20);
				chatTextDrawingArea.method382(0xffffff, l, "Play TrisidiaX", k1 + 5, true);
			}
			...
			if(loginScreenState == 3) {
				chatTextDrawingArea.method382(0xffff00, c / 2, "TrisidiaX Information", c1 / 2 - 60, true);
				int k = c1 / 2 - 35;
				chatTextDrawingArea.method382(0xffffff, c / 2, "Our Website: www.TrisidiaX.Betaboard.ca", k, true);
				k += 15;
				chatTextDrawingArea.method382(0xffffff, c / 2, "Please Vote for us DAILY with ::vote4cash", k, true);
				k += 15;
				chatTextDrawingArea.method382(0xffffff, c / 2, "Reporting Glitches on the Forum is Rewarded.", k, true);
				k += 15;
				chatTextDrawingArea.method382(0xffffff, c / 2, "To login, go back and Click Play TrisidiaX.", k, true);
				k += 15;
				int j1 = c / 2;
				int i2 = c1 / 2 + 50;
	...
	private void processLoginScreenInput() {
		//if(normalLogin == true) {
			if(loginScreenState == 0) {
				...
				if(super.clickMode3 == 1 && super.saveClickX >= i - 75 && super.saveClickX <= i + 75 && super.saveClickY >= l - 20 && super.saveClickY <= l + 20) {
					loginMessage1 = "   Welcome to TrisidiaX";
